...
class FluxKontextPipeline(
    ...
    def prepare_latents(
        ...
        if image is not None:
            ...
            image_latents_4d = image_latents
        ...
        return latents, image_latents, latent_ids, image_ids, image_latents_4d
    ...
    def prepare_inference(
        ...
        latents, image_latents, latent_ids, image_ids, image_latents_4d = self.prepare_latents(
            image,
        ...
        return latents,prompt_embeds,pooled_prompt_embeds,timesteps,image_latents,guidance,text_ids,latent_ids, ref_image
                ...
                if original:
                    ...
                    def choose_y0_by_snr(
                        ...
                        if save_time > 300:
                            ...
                            x0_pred_latent = (
                                                         x0_pred_latent / pipe.vae.config.scaling_factor) + pipe.vae.config.shift_factor
                            ...
                            xt_pred_latent = (
                                                         xt_pred_latent / pipe.vae.config.scaling_factor) + pipe.vae.config.shift_factor
                    ...
                    y0_pil = choose_y0_by_snr(self, start_noise, noise_pred, latents, image_latents_4d,save_time=save_time)
                    y0_pil.save(os.path.join(save_folder, f'time_{t}.png'))
                    ...
                    if save_dir is not None:
                        ...
                        timewise_image = image_[0]  # timewise_pil
                        original_image = source_image
                        # --------------------------------------------------------
                        # [1] GET SNR Value
                        img_np = _to_numpy_rgb(timewise_image)
                        ref_np = _to_numpy_rgb(original_image)
                        snr_value_db = _snr_db(img_np, ref_np)
                        with open(os.path.join(save_dir, "snr_log.csv"), "a", newline="") as f:
                            ## 왜 값이 크지 ???
                            writer = csv.writer(f)
                            if f.tell() == 0:
                                writer.writerow(["step", "snr_db"])
                            writer.writerow([int(i), float(snr_value_db)])

                        def set_compare_tensor(inference_step, save_time, x0_pred_pil, x_current_pil):
                            if inference_step > -1:
                                if save_time > 300:
                                    return x0_pred_pil
                                else:
                                    return x_current_pil
                        ...
                        timewise_image = set_compare_tensor(i, save_time, x0_pred_pil, timewise_image)
                        timewise_image.save(os.path.join(save_dir, str(i).zfill(6) + ".png"))
                    ...
                    latents, image_latents, change_num, sigmas, metrics = self.onetime_inference(latents=latents,
                                                                                                 ...
                                                                                                 pareto_construct=pareto_construct,
                                                                                                 tau_S=structure_ideal_low,
                                                                                                 tau_E=tau_E,
                                                                                                 ...
                                                                                                 max_pareto_iters=max_pareto_iters,
                                                                                                 controller=controller,
                                                                                                 args=args)

                    snr = metrics.get("snr")
                    vlm = metrics.get("vlm")
                    if pareto_construct and i == 0:
                        save_snr = snr
                        save_vlm = vlm
