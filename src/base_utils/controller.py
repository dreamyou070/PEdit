    ...
    if model_name == 'Kontext' :
        ...
        for nm, module in transformer.named_modules():
            ...
            if module.__class__.__name__ in ( 'Attention'):
                ...
                #txt_alpha = txt_alpha.clamp(0.2, 1.0)
                #img_alpha = img_alpha.clamp(0.2, 1.0)

                txt_alpha = torch.sigmoid(txt_alpha)
                img_alpha = torch.sigmoid(img_alpha)
                controller.set_alpha(str(attention_id), txt_alpha=txt_alpha, img_alpha=img_alpha)
...
def set_alpha_dict(model, transformer, case,
    ...
    if model == 'Qwen' :
        ...
        SIB = [5,6,7,19,22,29,37,43]
        STB = [1,8,14,29,31,35,38,47,49,57,58]
        EIB = [26,39,57]
        ETB = [49,40]
