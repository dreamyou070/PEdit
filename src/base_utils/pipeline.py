    ...
    def generate(self,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = latents
                if image_latents is not None:
                    latent_model_input = torch.cat([latents.to('cuda'), image_latents.to('cuda')], dim=1)
                timestep = t.expand(latents.shape[0]).to(latents.dtype)
                self.transformer.to('cuda')
                noise_pred = self.transformer(
                    hidden_states=latent_model_input,
                    timestep=(timestep / 1000).to('cuda'),
                    guidance=guidance,
                    pooled_projections=pooled_prompt_embeds.to('cuda'),
                    encoder_hidden_states=prompt_embeds.to('cuda'),
                    txt_ids=text_ids.to('cuda'),
                    img_ids=latent_ids.to('cuda'),
                    joint_attention_kwargs=self.joint_attention_kwargs,
                    return_dict=False,
                )[0]
                noise_pred = noise_pred[:, : latents.size(1)]
                ...
                if record_file is not None :
                    ...
                    def _snr_db(img, ref, eps=1e-12):
                        # both HxWxC in [0,1]
                        #if img.shape != ref.shape:
                        #    ref_pil = Image.fromarray((ref * 255).astype(np.uint8))
                            #ref_pil = ref_pil.resize((img.shape[1], img.shape[0]), Image.BICUBIC)
                        ref = ref.resize((512, 512), resample=Image.Resampling.LANCZOS)
                        ...
                        sig = np.mean(ref ** 2)
                        return 10.0 * math.log10((sig + eps) / (mse + eps))

                    snr = _snr_db(y0_pil, source_image, eps=1e-12)
