        ...
        for file_idx, filename in enumerate(os.listdir(src_folder)):
            if file_idx == trg_file_idx:
                ...
                if not os.path.exists(unique_folder):
                    ...
                    src_img.save(os.path.join(unique_folder, f'source.png'))
                    with pipe.progress_bar(total=num_steps) as progress_bar:
                        for stage_idx, t in enumerate(timesteps):

                            def do_stage1(lat_map_cpu, t, src_latents, image_latents_4d, base_latents):
                                    ...
                                    if cfg_name != 'case1':
                                        with torch.no_grad():
                                            set_alphas_for_case(pipe.transformer,
                                                                ...
                                                                cfg_name,
                                                                config_dict)
                                            _, noise_pred, metrics, timewise_pil = pipe.onetime_inference_qwen(latents=to_like(lat_map_cpu[cfg_name]),
                                            ...
                                            timewise_pil = self.sd.pipeline.vae.decode(timewise_pil, return_dict=False)[0][
                                                :, :, 0]
                                            timewise_pil = self.sd.pipeline.image_processor.postprocess(timewise_pil,output_type='pil')[0]
                                ...
                                for cfg_name in config_dictionary:
                                    ...
                                    if cfg_name == 'case1':
                                        ...
                                        timewise_metrics[cfg_name] = {"snr": snr_score, 'tca_ratio': tca_ratio}
                                        snr2 = timewise_metrics['case2']['snr'].to(device_ref,dtype=torch.bfloat16).detach()
                                        snr3 = timewise_metrics['case3']['snr'].to(device_ref,dtype=torch.bfloat16).detach()
                                        target_snr = (snr2 + snr3) / 2.0
                                        target_diff1 = abs(snr_score - target_snr)
                                        tcr2 = timewise_metrics['case2']['tca_ratio'].to(device_ref,dtype=torch.bfloat16).detach()
                                        tcr3 = timewise_metrics['case3']['tca_ratio'].to(device_ref,
                                                                                         dtype=torch.bfloat16).detach()
                                        target_tca = (tcr2 + tcr3) / 2.0
                                        ...
                                        if target_diff1 > target_diff2:
                                            ...
                                            target = target_tca
                                            diff_2 = torch.abs(tcr2 - tca_ratio)
                                            diff_3 = torch.abs(tcr3 - tca_ratio)
                                            if diff_2 > diff_3:
                                                following = 'case2'
                                            else:
                                                following = 'case3'
                                            ...
                                            check_snr = False
                                        def build_alpha_params(config_dict,train_names,device=None,):
                                        ...
                                        alpha_params = build_alpha_params(config_dict,train_set,device=device_ref)
                                        ...
                                        for epoch in range(max_epochs):
                                            ...
                                            # ------------------------------------------------------------------
                                            # 여기에 매 epoch 마다 log 를 보여주고 싶어
                                            # ------------------------------------------------------------------
                                            accelerator.print(
                                                f"[cfg={cfg_name}] stage={stage_idx} epoch={epoch} | "
                                                f"snr={float(snr_score):.4f}  "
                                                f"tca={float(tca_ratio):.4f}  "
                                                f"snr2={float(snr2):.4f}  "
                                                f"snr3={float(snr3):.4f}  "
                                                f"tcr2={float(tcr2):.4f}  "
                                                f"tcr3={float(tcr3):.4f}  "
                                                f"loss_main={float(loss_main):.4f}  "
                                                f"loss_reg={float(loss_reg):.4f}  "
                                            ...
                                            if stage_idx < stage_1:
                                                ...
                                                stop_sig = 0
                                                if snr2 > snr3 :
                                                    if snr2 > snr1 and snr1 > snr3 :
                                                        stop_sig += 1
                                                if snr2 < snr3 :
                                                    if snr2 < snr1 and snr1 < snr3 :
                                                        stop_sig += 1
                                                if tca2 > tca3 :
                                                    if tca2 > tca1 and tca1 > tca3 :
                                                        stop_sig += 1
                                                if tca2 < tca3:
                                                    if tca2 < tca1 and tca1 < tca3:
                                                        stop_sig += 1
                            ...
                            if stage_idx  < stage_1 :
                                noise_pred_dict, lat_map_cpu = do_stage1(lat_map_cpu, t, src_latents, image_latents_4d, base_latents)
                                ...
                                if search:
                                    ...
                                    if tca2 > tca3 :
                                        ...
                                        target_tca = (tca2 < tca_ratio) and (tca3 > tca_ratio)
                                    accelerator.print(
                                        f"[cfg={cfg_name}] stage={stage_idx} | "
                                        f"snr={float(snr_score):.4f}  "
                                        f"tca={float(tca_ratio):.4f}  "
                                    )
                                    ...
                                    if target_snr and not target_tca :
                                        # [1]
                                        if tca2 > tca3 :
                                            if tca_ratio > tca2 : # if too editing, get midium between 3
                                                config_dictionary['case1'] = set_midium(config_dictionary['case1'],config_dictionary['case3'])
                                            elif tca_ratio < tca3 : # get medium between 2
                                                config_dictionary['case1'] = set_midium(config_dictionary['case1'],
                                                                                        config_dictionary['case2'])
                                        if tca2 < tca3 :
                                            if tca2 > tca_ratio : # get midium between 3
                                                config_dictionary['case1'] = set_midium(config_dictionary['case1'],
                                                                                        config_dictionary['case3'])

                                            elif tca3 < tca_ratio : # get midium between 2
                                                config_dictionary['case1'] = set_midium(config_dictionary['case1'],
                                                                                        config_dictionary['case2'])

                                    if not target_snr and target_tca:
                                        if snr2 > snr3 :
                                            if snr3 > snr1 : # too low preserve, get medium between 2
                                                config_dictionary['case1'] = set_midium(config_dictionary['case1'],
                                                                                        config_dictionary['case2'])

                                            if snr1 > snr2 : # too preserve, get medium between 3
                                                config_dictionary['case1'] = set_midium(config_dictionary['case1'],
                                                                                        config_dictionary['case3'])

                                        if snr2 < snr3 :
                                            if snr1 < snr2 : # too low preserve, get medium between 2
                                                config_dictionary['case1'] = set_midium(config_dictionary['case1'],
                                                                                        config_dictionary['case2'])

                                            if snr3 < snr1 : # get medium between 3
                                                config_dictionary['case1'] = set_midium(config_dictionary['case1'],
                                                                                        config_dictionary['case3'])

                                    if not target_snr and not target_tca :
                                        #print(f' go stage1 again ...')
                                        #noise_pred_dict, lat_map_cpu = do_stage1(lat_map_cpu,
                                        #                                         t,
                                        #                                         src_latents,
                                        #                                         image_latents_4d,
                                        #                                         base_latents)
